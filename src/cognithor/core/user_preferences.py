...
Uses the same DB as SessionStore (~/.cognithor/sessions.db).
"""
...
class UserPreferenceStore:
    ...
    Uses the same database as SessionStore (~/.cognithor/sessions.db)
    ...
    def __init__(self, db_path: str | Path | None = None) -> None:
        if db_path is None:
            db_path = Path.home() / ".cognithor" / "sessions.db"
