...
class MemoryEncryptor:
    ...
    def _load_key(self) -> None:
        ...
        try:
            ...
            logger.error("IMP_ARWEAVE_ENCRYPTION_KEY is not valid hex — encryption disabled.")
    ...
    def decrypt(self, envelope: dict) -> Optional[str]:
        ...
        if self._key is None:
            logger.error("Cannot decrypt Arweave record: IMP_ARWEAVE_ENCRYPTION_KEY not set.")
            return None
        ...
        try:
            ...
            logger.error("Arweave decryption failed (wrong key or tampered data): %s", e)
            return None
...
class ArweaveStore:
    ...
    def __init__(
        ...
        if self.wallet_path and os.path.exists(self.wallet_path):
            ...
            logger.warning(f"Arweave wallet not found: {self.wallet_path}. Read-only mode active.")
        ...
        try:
            ...
            if isinstance(raw, dict) and raw.get("encrypted") is True:
                ...
                if plaintext is None:
                    logger.error(
                        "Failed to decrypt Arweave record %s — wrong key or tampered data.",
                        tx_id[:16],
                    )
                    return None
